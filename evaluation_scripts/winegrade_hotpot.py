...
OUTPUT_DIR = "results_hotpotqa_winogrande_strict"
...
def extract_hotpot_answer_strict(text: str) -> Tuple[Optional[str], str]:
    """
    Strict short-answer parser:
    - Accepts explicit 'Final answer: ...' anywhere
    - Accepts last-line 'Answer: ...' or 'The answer is ...'
    - Does NOT accept arbitrary non-empty last lines
    """
    out = text or ""

    payload_any, src_any = extract_final_answer_anywhere(out)
    if payload_any is not None:
        ans = payload_any.strip()
        if ans:
            return ans, f"strict_{src_any}_text"

    payload_last, src_last = extract_last_line_payload(out)
    p_last = payload_last.strip()

    m = re.match(r"(?is)^\s*(?:answer:|the\s+answer\s+is)\s*(.+?)\s*$", p_last)
    if m:
        ans = m.group(1).strip()
        if ans:
            return ans, f"strict_{src_last}_answer_phrase"

    return None, "unparsed_none_hotpot_strict"
...
def make_short_answer_prompt(
    ...
    if is_deepseek_model(model_name):
        return (
            ...
            "Do not explain your reasoning.\n"
            "Do not output anything except the final line.\n"
            "Output exactly one line in this format: Final answer: <answer>\n\n"
            f"{body}"
        )
    ...
    return (
        f"{instruction}\n"
        "Do not provide reasoning.\n"
        "Return exactly one line and nothing else in this format: Final answer: <answer>\n\n"
        f"{body}"
    )
...
def build_hotpotqa_item(sample: Dict[str, Any], model_name: str) -> Optional[Dict[str, Any]]:
    ...
    body = f"Question:\n{question}"
    prompt = make_short_answer_prompt(
    ...
    return {
        "prompt": prompt,
        "reference": ref,
        "parse": extract_hotpot_answer_strict,
        "score": lambda pred, ref: normalize_span(pred) == normalize_span(ref),
    }
...
DATASETS = [
    {
        "name": "hotpotqa",
        "load_specs": [
            {"path": "hotpotqa/hotpot_qa", "config": "distractor", "split": "validation"},
        ],
        "build": build_hotpotqa_item,
        "max_new_tokens": 32,
    },
    {
        "name": "winogrande",
        "load_specs": [
            {"path": "allenai/winogrande", "config": WINOGRANDE_CONFIG, "split": "validation"},
        ],
